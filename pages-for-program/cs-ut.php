...
//list: ratings and school
  ...
  $stmt = $pdo->prepare("SELECT * FROM `reviews` WHERE `school` = 'University of Toronto' AND `program` = 'Computer Science' ORDER BY `id` DESC");
  $stmt->execute();
  $stmtt = $pdo->prepare("SELECT AVG(rating) FROM `reviews` WHERE `school` = 'University of Toronto'");
  $stmtt->execute();
  $row = $stmtt->fetch()
  ?>
  <!DOCTYPE html>
  ...
  <a class="accbtn" href="https://www.wlu.ca/programs/science/undergraduate/chemistry-bsc/program-details.html
">Program details</a>
