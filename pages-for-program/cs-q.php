...
//list: ratings and school
  ...
  $stmt = $pdo->prepare("SELECT * FROM `reviews` WHERE `school` = 'Queens University' AND `program` = 'Computer Science' ORDER BY `id` DESC");
  $stmt->execute();
  $stmtt = $pdo->prepare("SELECT AVG(rating) FROM `reviews` WHERE `school` = 'Queens University'");
  $stmtt->execute();
  $row = $stmtt->fetch()
  ?>
  <!DOCTYPE html>
