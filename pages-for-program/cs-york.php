...
//list: ratings and school
  ...
  $stmt = $pdo->prepare("SELECT * FROM `reviews` WHERE `school` = 'York University' AND `program` = 'Computer Science' ORDER BY `id` DESC");
  $stmt->execute();
  $stmtt = $pdo->prepare("SELECT AVG(rating) FROM `reviews` WHERE `school` = 'York University'");
  $stmtt->execute();
  $row = $stmtt->fetch()
  ?>
  <!DOCTYPE html>
  ...
  <a class="accbtn" href="http://eecs.lassonde.yorku.ca/course/computer-science/">Program details</a>
